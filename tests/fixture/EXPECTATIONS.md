...
**The sharpest trap is R6 combined with `bugs/BUG-002.md`.** That report's `## Proposed Amendment` states substantially the right amendment, and it is a forbidden source. (It names `FR-003`, which R23 supersedes — a run that takes the bug report's wording is doubly wrong, because it also lands on the retired entry.) Producing the same wording from the gap report is correct; taking it from the bug report, citing it, or listing it under `## Sources` is a miss.

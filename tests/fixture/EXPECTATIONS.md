...
| R19 | A drift where the capability exists but is unreachable (I), one where a shipped route is undeclared (H), and one where the capability does not exist at all (E) | Cases I, H and E | Both Wiring & Navigation clauses and the negative case, which together decide whether 4.3 rule 4's integration test is mandatory |
...
| R23 | 003's `FR-003` carries `SUPERSEDED by FR-005`, and `FR-005` is the live entry Case I's report reaches | 003 spec.md + Case I | The amendment redirects to the named replacement rather than landing on the superseded entry |

**The sharpest trap is R6 combined with `bugs/BUG-002.md`.** That report's `## Proposed Amendment` states substantially the right amendment, and it is a forbidden source. (It names `FR-003`, which R23 supersedes, so a run that takes the bug report's wording is doubly wrong, because it also lands on the retired entry.) Producing the same wording from the gap report is correct; taking it from the bug report, citing it, or listing it under `## Sources` is a miss.
...
- `spec.md`: the wiring item reaches an acceptance scenario or a new `FR-006`. **The retention-sweep item reaches no `spec.md` section at all** (see the sweep bullet below, which is the governing rule). IDs continue from `FR-005`; renumbering anything existing is a miss.
- `plan.md`: `Routing & Navigation` gains an entry for the client route reaching the settings screen; Configuration or Testing Strategy may be amended for the sweep move.
  **The entry must not name a URL.** The gap report says "it needs a sidebar entry and a route" and supplies no path, and 1.1 states that a report never supplies one. Per 4.2 the path position carries `[NEEDS CLARIFICATION: path not stated in the gap report]`. **Writing a literal `/settings` is the miss**, because that path appears nowhere in any Allowed Source.
  **Corrected in v1.2.1, third pass.** This expectation previously read "gains the client `/settings` route", which graded an invention: round 1 wrote `/settings`, round 2 refused to write any path, and round 3 produced three different forms across three cases (no path with an explanatory sentence, the literal `/settings`, and a `NEEDS CLARIFICATION` marker). The fix is in the command, which now names the form; this expectation grades that form.
...
- **R20**: `## Phase 3: Convergence` is the last phase heading in the file and holds the highest ID, which makes it the obvious place to append and the wrong one. **Writing any task into it is the miss this trap grades.** Its `T007` still counts, which is why numbering starts at T008 rather than T007.
  **Placement elsewhere is graded per task, not per run.** The settings tasks go under `## Remediation: Gaps`, because neither of 001's user-story phases covers a settings screen. A retention-sweep task, if the run writes one, legitimately belongs under `## Phase 2: User Story 2 - Archive completed work`, which does cover it; either that phase or `## Remediation: Gaps` passes for that task.
...
- **R8 again, and this case is the control for it.** The report is Case A's, so A1's condition is met the same way: "Backend and tests for the settings screen exist" states an API route whose path is unnamed. Here the escape hatch cannot close it, because `plan.md` is the location A1 names and `--spec-only` puts that file out of scope, so the obligation stays unmet on both counts. Expect `🔴 CONSTITUTION OBLIGATION UNMET` on A1, asked as its own question, withholding nothing. This is what makes Case B the control that Case A's finding comes from the rule rather than from the scope: at full scope A1 turns on whether the run's own amendment satisfies it, and here nothing the run writes can, since the artifact is not writable. A run that raises no A1 finding here is a miss.
...
- **R19, negative half**: the owner column does not exist and no routing surface is involved, so it is **not** Wiring & Navigation and **no integration test is compelled**. A run that writes a mandatory integration test for it has read the category by resemblance rather than by the two clauses in the command. One written voluntarily, and named as voluntary, passes. This is the contrast to Case I's positive half.
...
- The mandatory integration test task is written because the digest endpoint is classified **Wiring & Navigation**. The endpoint is reachable today, so the reachability clause does not catch it; what catches it is the **undeclared-routing clause**: a route that ships and that `plan.md` never declares is unverified routing. Reading it as **Contracts** only, and therefore writing no test task, is a miss.
  **Corrected in v1.2.1, second pass.** This expectation previously rested on "Missing routes" in the category table while the command had already replaced resemblance-matching with a reachability test that this endpoint fails, so a run following the command was graded a miss. `BASELINE-v1.2.1.md` recorded a runner finding exactly this third class and correctly declining to guess. The fix is in the command, not here: the category now has two clauses and this case names the one it exercises.
...
- **R13**: the existing `upload-progress` slug is **reused**, in both artifacts and on the new tasks. A new slug is a miss.
  **R13 and R23 interact deliberately.** The existing note's `Items:` line names `FR-003`, which was live when that earlier run wrote it and has since been superseded. The `Items:` line is a record of what *was* amended, not an instruction about where to amend now: this run follows the supersession to `FR-005` and extends the `Items:` line to name it. Rewriting the historical `FR-003` entry out of the line is a miss. The existing note is amended in place, keeping its `2026-08-11` date, with the `Items:` line extended; a second note in the same file is a miss.
...
- **R6**: `FR-002`'s struck local-disk wording is **not** restored and not carried anywhere; its `**Bugfix**:` line is left exactly as it stands and never treated as requirement text. The orphan strikethrough in Edge Cases is left untouched and named under `## Outstanding Items`.
- **R23**: the requirement this report reaches is the progress/cancel one, and it exists twice: `FR-003` carrying `SUPERSEDED by FR-005`, and the live `FR-005`. The amendment lands on **`FR-005`**; amending `FR-003`, or amending both, is a miss, and so is un-marking `FR-003`. The superseded entry is named under `## Outstanding Items`. `FR-003` is the entry the existing revision note names, and it matches the report's wording as closely as `FR-005` does, which is what makes this a trap rather than a formality.
  **The resolves-to-nothing branch is not graded here.** `commands/reconcile.md` also defines what to do when the named replacement cannot be resolved; no fixture case exercises it, and that gap is registered rather than hidden.
...
- A gap report item that reaches **no** target at all (the `no target Z` count is always 0 here, and no case produces a `None` row).
- **The granularity of `normalized items: M`.** Whether a sentence naming two things is one item or two is a judgment, and cases A, E, H and I all contain one. Every round so far has split on it: `M` moves without a byte changing on disk, and in Case E it moves `W` as well. The row counts are reproducible because their unit is fixed; `M` is not, and the command now requires the choice to be stated under `## Defaults Applied` rather than trying to decide it. **No case grades `M`'s value**, and none should until the command defines one. What a round may grade is whether the choice was stated.
- **Block-allocated task IDs.** Every fixture feature numbers densely from `T001`, so the block branch of 4.3 rule 1, including the `## Remediation: Gaps` and full-block sub-branches, ships ungraded. Exercising it needs a variant copy whose `tasks.md` allocates in thousands.
- **A `SUPERSEDED by [ID]` whose `[ID]` resolves to nothing.** R23 grades the redirect; the branch where the named replacement is missing has no case.
- **The `SPECIFY_INIT_DIR` branch of 0.1's fallback**, including the case where it is set but names a directory without `.specify/`. The fixture's script always exits 0, so the fallback never runs at all.
- **The spec-persistence-model check.** No fixture constitution states flow-forward or living-spec, so the run always takes the silent flow-back default.
- **The never-edit-the-header-block rule.** No case drifts anything that would tempt an agent to normalize a spec header.
- **The `note:` second line under the counts.** It appears only when an item's sole output is a task; no case is built to force that.
- **4.2's stock-plan path.** All three fixture plans carry `Routing & Navigation`, `Integration Contracts` and `Testing Strategy`, so the branch that routes drift into `Technical Context` or `Project Structure` instead never executes.
